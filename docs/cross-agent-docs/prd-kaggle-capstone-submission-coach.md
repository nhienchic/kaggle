...
The GitHub connector could not access `nhienchic/kaggle`, and `gh` is not installed in this environment. When GitHub access is available, publish this PRD with:

```powershell
gh issue create --repo nhienchic/kaggle --title "PRD: Kaggle Capstone Submission Coach" --label "ready-for-agent" --body-file docs/cross-agent-docs/prd-kaggle-capstone-submission-coach.md
```
